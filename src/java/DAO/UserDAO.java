...
public class UserDAO {
    ...
    //----------------------------INSERT----------------------------------------
    public static int addNewUser(String name, String email, int permission, String password, String description) {
        int rc = 0;
        try {
            SimpleDateFormat dFormate = new SimpleDateFormat("MMM dd yyyy HH:mma");
            Date date = new Date();
            Connection conn = DatabaseHelper.openConnection();
            String sql = "Insert into users(users_name, email, permission, user_password, date_modify, description) values (?, ?, ?, ?, ?, ?)";
    ...
    public ArrayList<User> getAllUsers() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                allUsers.add(new User(userId, permission, name, email, password, dateModify, description));
            }
            conn.close();
    ...
    //----------------------------UPDATE----------------------------------------
    public int updateUser(int userId, String name, String email, int permission, String password, String description) {
        int rc = 0;
        try {
            SimpleDateFormat dFormate = new SimpleDateFormat("MMM dd yyyy HH:mma");
            Date date = new Date();
    ...
    public User searchByID(int id) {
        try {
            ...
            while (rs.next()) {
                ...
                Date dateModify = dFormate.parse(rs.getString(6));
                String description = rs.getString(7);
                return new User(userId, permission, name, email, password, dateModify, description);
            }
            conn.close();
    ...
    public User checkLogin(String email, String password) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                Date dateModify = dFormate.parse(rs.getString(6));
                String description = rs.getString(7);
                user = new User(userId, permission, name, email, password, dateModify, description);
            }
            conn.close();
            System.out.println("Get users by name successfully!");
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(dao.updateUser(1, "Như Khang", "khangvo@gmail.com", 2, "123456", "update"));
    }
}
